...
.pull-left[
```{r echo=FALSE, out.width="100%", warning = F, message = F}
email %>%
...
```{r echo=FALSE, out.width="70%", warning = F, message = F}
means <- email %>%
  group_by(spam) %>%
  summarise(mean_num_char = mean(num_char)) %>%
  mutate(group = 1)
...
.pull-left[
```{r spam-predict-viz, echo=FALSE, out.width = "100%", fig.asp=0.5,  warning = F, message = F}
newdata <- tibble(
  num_char = c(2, 15, 40),
...
```{r ref.label="spam-predict-viz", echo=FALSE, fig.asp=0.5,  warning = F, message = F}
```
